...
After installing the package, run the bin script to copy the docs into your project:

```bash
npx graphics-components-llm-docs
```

This copies `dist/llm-docs/` to `./llms/graphics-components/` in your project root and creates or updates `CLAUDE.md` with a pointer to the docs so agents can find them automatically. Run it again after upgrading the package to refresh the docs.

To copy to a custom location, pass a path as the first argument:

```bash
npx graphics-components-llm-docs ./my/custom/path
```
...
```json
[
  { "srcPath": "guides/archieml.mdx", "destFile": "archieml.md", "title": "Using ArchieML" }
]
```
