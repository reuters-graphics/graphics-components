...
Use the `onMapReady` callback to customize the map's appearance after it loads:

<Canvas of={MapStories.CustomStyledMap} />

## Adding GeoJSON layers

Use the `MapLayer` component to add GeoJSON data to your map:
...
You can add multiple `MapLayer` components to display different GeoJSON datasets with different styling.
