...
{#snippet esPublished()}
  Publicado <time datetime={new Date(content.publishTime).toISOString()}>
    {new Date(content.publishTime).toLocaleDateString('es-ES', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })}&nbsp;&nbsp;{formatApTime(new Date(content.publishTime), {
...
Two details worth copying from that snippet: bind `datetime` to the same instant
you format, so the machine-readable attribute and the visible text can't drift
apart, and pass `timeZone` explicitly when you want a dateline pinned to one
city rather than resolved against each reader's own clock.
